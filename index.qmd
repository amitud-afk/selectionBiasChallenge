...
img_path = 'Amit.jpeg'  # Example image - replace with your own image
...
## Selection Bias Meme
![Four-panel statistics meme showing Reality (original image), My Model (stippled version), Selection Bias (letter S), and Estimate (masked stippled image)](my_statistics_meme.png)

This meme demonstrates selection bias by showing how systematic missing data patterns distort our understanding of reality. The original image (Reality) represents the true population, while the stippled version (Your Model) shows our data collection through sampling. When selection bias removes data points in a systematic "S" pattern, the resulting estimate becomes biased and no longer accurately represents the true population, just as missing data in real-world studies can lead to incorrect conclusions when the missingness follows a non-random pattern.
